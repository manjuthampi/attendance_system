            ...
            <?php  
                ...
                for($cb=1;$cb<=$boxDisplay;$cb++){ 
                    if(($cb >= $currentMonthFirstDay+1 || $currentMonthFirstDay == 7) && $cb <= ($totalDaysOfMonthDisplay)){ 
                        ...
                        $result = $db->query("SELECT * FROM tbl_events WHERE event_date = '".$currentDate."'"); 
...
function getEvents($date = ''){ 
    ...
    $result = $db->query("SELECT * FROM tbl_events WHERE event_date = '".$date."'"); 
    if($result->num_rows > 0){ 
        $eventListHTML = '<h2>Events on '.date("l, d M Y",strtotime($date)).'</h2>'; 
        $eventListHTML .= '<ul>'; 
        while($row = $result->fetch_assoc()){  
            $eventListHTML .= '<h3 style="color:#D65E02;margin-left:-80px;">'.'<ul>'.$row['event_name'].'</ul>'.'</h3>'; 
			$eventListHTML .= '<p align="justify">'.$row['remarks'].'</p>';
        } 
        $eventListHTML .= '</ul>'; 
    } 
    echo $eventListHTML; 
